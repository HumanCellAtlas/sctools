...
def test_split_bam_raises_value_error_when_passed_bam_without_barcodes(bamfile):
    ...
    with pytest.raises(RuntimeError):
        bam.split(bamfile, 'test_output', consts.CELL_BARCODE_TAG_KEY, approx_mb_per_split=split_size)
...
def test_split_on_tagged_bam(tagged_bam):
    ...
    outputs = bam.split(
        tagged_bam, 'test_output', consts.CELL_BARCODE_TAG_KEY, consts.RAW_CELL_BARCODE_TAG_KEY,
...
def test_split_with_raise_missing_true_raises_warning_without_cr_barcode_passed(tagged_bam):
    ...
    with pytest.raises(RuntimeError):
        outputs = bam.split(
            tagged_bam, 'test_output', consts.CELL_BARCODE_TAG_KEY, approx_mb_per_split=split_size, raise_missing=True)
...
def test_split_succeeds_with_raise_missing_false_and_no_cr_barcode_passed(tagged_bam):
    split_size = 1024  # our test data is very small, this value should yield 1 file
    outputs = bam.split(
        tagged_bam, 'test_output', consts.CELL_BARCODE_TAG_KEY, approx_mb_per_split=split_size, raise_missing=False)
...
def test_sort_by_tags_and_queryname_sorts_correctly_from_file():
    ...
    tag_sortable_records = (bam.TagSortableRecord.from_aligned_segment(r, tag_keys) for r in sorted_records)
...
def test_tag_sortable_record_lt_is_false_for_equal_records():
    r1 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='A')
    r2 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='A')
    assert not r1 < r2


def test_tag_sortable_record_lt_is_true_for_smaller_query_name():
    r1 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='A')
    r2 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='B')
    assert r1 < r2


def test_tag_sortable_record_lt_is_true_for_smaller_tag():
    r1 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='A')
    r2 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'B'], query_name='A')
    assert r1 < r2
    r1 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='A')
    r2 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'B', 'A'], query_name='A')
    assert r1 < r2
    r1 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='A')
    r2 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['B', 'A', 'A'], query_name='A')
    assert r1 < r2


def test_tag_sortable_record_lt_is_true_for_smaller_tag_regardless_of_query_name():
    r1 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='B')
    r2 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'B'], query_name='A')
    assert r1 < r2


def test_tag_sortable_record_lt_empty_query_name_is_smaller():
    r1 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='')
    r2 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='A')
    assert r1 < r2


def test_tag_sortable_record_lt_empty_tag_is_smaller():
    r1 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', ''], query_name='A')
    r2 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='A')
    assert r1 < r2
    r1 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', '', 'A'], query_name='A')
    r2 = bam.TagSortableRecord(tag_keys=TAG_KEYS, tag_values=['A', 'A', 'A'], query_name='A')
...
def make_records_from_values(tag_keys, tags_and_query_name):
    ...
    for i in range(len(tags_and_query_name)):
        r = bam.TagSortableRecord(tag_keys=tag_keys, tag_values=tags_and_query_name[i][0], query_name=tags_and_query_name[i][1])
        records.append(r)
    return records
