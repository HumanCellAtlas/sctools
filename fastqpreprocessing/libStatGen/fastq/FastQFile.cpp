                                        ...
                                        BaseAsciiMap::SPACE_TYPE spaceType)
{
   ...
   if(status == FastQStatus::FASTQ_SUCCESS)
   {
      ...
      // Open the file
      myFile = ifopen(fileName, "rt", InputFile::GZIP);

      //printf("KK going to open file in FastQFile.cpp  103 %d \n", myFile);
      myFileName = fileName;
   ...
   else
   {
       ...
       if(myCheckSeqID)
       {
           ...
           std::pair<std::map<std::string, unsigned int>::iterator,bool> insertResult;
            /*
           insertResult = myIdentifierMap.insert(std::make_pair(mySequenceIdentifier.c_str(), 
                                                     myLineNum));
